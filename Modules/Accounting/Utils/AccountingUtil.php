...
class AccountingUtil extends Util
{
    ...
    public static function createJournalEntry($type, $business_id, $location_id, $user_id, $operation_date, $link_table, $link_id, $recs, $note = '')
    {
        try {
                
            DB::beginTransaction();
    
            // ✅ Instantiate necessary utilities
            ...
            // ✅ Ensure all records have required fields before inserting
    ...
    public static function getAmountsAcounts($products)
    {
        ...
        foreach ($products as $one)
        {
            ...
            $discount = $discount *  self::r_s($one['quantity']);
    ...
    public static function getAmountsAcounts2($sell_lines)
    {
        ...
        foreach ($sell_lines as $one)
        {

            if($one->quantity_returned != 0)
            {
                $customer = 0;
                $tax = 0;
                $discount = 0;
                ...
                $customer = $one->unit_price_inc_tax * $one->quantity_returned;

                $tax = $one->item_tax * $one->quantity_returned;

                $discount = $one->line_discount_amount;
                if($one->line_discount_type == 'percentage')
                    $discount = $one->unit_price * ($discount / 100);
                $discount = $discount * $one->quantity_returned;

                $revenue_of_products = $customer - $tax + $discount;

                if($one->product->enable_stock == 1)
                {
                    $variation_product = $one->variations;
                    $cost_of_goods = $variation_product->default_purchase_price * $one->quantity_returned;
                }
    ...
    public static function getAmountsAcounts4($purchases)
    {
        ...
        foreach ($purchases as $one)
        {
            ...
            $one_supplier = $util->num_uf($one['purchase_price_inc_tax']);

            $one_stock += $util->num_uf($one['pp_without_discount']);

            if($one['discount_percent'] > 0)
                $one_discount += $util->num_uf($one['pp_without_discount']) - $util->num_uf($one['purchase_price']);


            if($one['item_tax'] > 0)
                $one_tax += $util->num_uf($one['item_tax']);


            $one_stock =  $one_stock * $one['quantity'];
            $one_discount =  $one_discount * $one['quantity'];
            $one_tax =  $one_tax * $one['quantity'];
            $one_supplier = $one_supplier * $one['quantity'];
            ...
            if($quantity > 0)
            {
                ...
                foreach ($purchase_lines as $purchase_line)
                {
                    if($purchase_line->id == $id)
                    {
                        ...
                        $one_stock =  $one_stock * $quantity;
                        $one_discount =  $one_discount * $quantity;
                        $one_tax =  $one_tax * $quantity;
                        $one_supplier = $one_supplier * $quantity;
