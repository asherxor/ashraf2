    ...
    public function show($id)
{
    ...
    // ✅ First, try to find an account where $id is a link_id
    $account = AccountingAccount::where('business_id', $business_id)
        ->where('link_table', 'accounts')
        ->where('link_id', $id)
        ->first();

    // ✅ If no account is found using link_id, fallback to using it as accounting_account_id
    if (!$account) {
        $account = AccountingAccount::where('business_id', $business_id)
            ->where('id', $id)
            ->first();
    ...
    // ❌ If still no account is found, return an error
    if (!$account) {
    ...
    if (request()->ajax()) {
        \Log::info('Fetching transactions for ID: '.$id.', resolved account ID: '.$accounting_account_id);

        // ✅ Fetch transactions for the resolved account ID
        $transactions = AccountingAccountsTransaction::where('accounting_account_id', $accounting_account_id)
        ...
        if (request()->filled('start_date') && request()->filled('end_date')) {
            ...
            $transactions->whereBetween('accounting_accounts_transactions.operation_date', [$start_date, $end_date]);
        }

        // ✅ Apply type filter (debit, credit)
        if (request()->filled('transaction_type') && in_array(request('transaction_type'), ['debit', 'credit'])) {
            $transactions->where('accounting_accounts_transactions.type', request('transaction_type'));
        }
        ...
        Log::info("Total Debit: " . $total_debit . " | Total Credit: " . $total_credit . " | Total Balance: " . $total_balance);

        return DataTables::of($data)
            ->editColumn('balance', function ($row) {
                return "<span amount='{$row->balance}' class='movement_balance_row'>{$this->commonUtil->num_f($row->balance, true)}</span>";
            })
            ->editColumn('operation_date', function ($row) {
                return $this->commonUtil->format_date($row->operation_date, true);
            })
            ->editColumn('note', function ($row) {
                return $row->trans_note ?: $row->note;
            })
            ->addColumn('debit', function ($row) {
                return $row->type == 'debit' ? "<span class='debit' data-orig-value='{$row->amount}'>{$this->commonUtil->num_f($row->amount, true)}</span>" : '';
            })
            ->addColumn('credit', function ($row) {
                return $row->type == 'credit' ? "<span class='credit' data-orig-value='{$row->amount}'>{$this->commonUtil->num_f($row->amount, true)}</span>" : '';
            })
            ->with([
    ...
    public function getAccountBalance($id)
    {
        ...
        // ✅ First, find the account using `link_id`, fallback to `id`
        $account = AccountingAccount::where('business_id', $business_id)
        ...
        // ✅ Use the same SQL logic as `getAllBalance()`
        $balance = AccountingAccountsTransaction::where('accounting_account_id', $accounting_account_id)
            ->select(DB::raw("
                SUM(CASE 
                    WHEN type = 'debit' THEN COALESCE(amount, 0) 
                    WHEN type = 'credit' THEN -1 * COALESCE(amount, 0) 
                    ELSE 0 
                END) AS balance
            "))
            ->first();
    
        return response()->json(['balance' => $balance->balance ?? 0]); // ✅ Secure JSON response
    }
