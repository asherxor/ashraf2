...
<script>
  $(document).ready(function() {
    $('#is_reverse').change(function() {
      let isReverse = $(this).val();
      let amountField = $('input[name="amount"]');
      let currentAmount = parseFloat(amountField.val());

      if (isReverse == 1) {
        amountField.val(-Math.abs(currentAmount)); // Convert to negative if it's a refund
      } else {
        amountField.val(Math.abs(currentAmount)); // Convert to positive if it's a normal payment
      }
    });

    // Ensure amount sign updates on input change
    $('input[name="amount"]').change(function() {
      $('#is_reverse').trigger('change');
    });
  });
</script>
